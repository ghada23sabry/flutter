"""Dev-only provisioning for the live AI RBAC verification user.

Creates a system `ai_viewer` role holding ONLY `ai.view`, an
`ai-viewer@acme.com` user in the `acme` tenant, and a store-scoped role grant
to the `Downtown` store. Used by `test/live_ai_test.dart` to prove the
ai.view read / ai.reconcile + ai.scan + ai.confirm mutation permission
boundary. Idempotent.

Usage (cwd = services/api, .env present):
  python -m scripts.provision_rbac_ai_viewer
  python -m scripts.provision_rbac_ai_viewer --email ai-viewer@example.com --password secret
...
async def provision(*, email: str, password: str) -> int:
    async with SessionLocal() as db:
        ...
        role = (
            await db.execute(select(Role).where(Role.tenant_id.is_(None), Role.name == "ai_viewer", Role.is_system.is_(True)))
        ).scalar_one_or_none()
        if role is None:
            role = Role(name="ai_viewer", description="System viewer role (read-only AI scans)", is_system=True)
            db.add(role)
            await db.flush()
    ...
    print(f"role=ai_viewer permissions={VIEWER_PERMISSIONS}")
    print(f"user={email} tenant={tenant.slug} store={store.name} store_id={store.id}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(prog="provision-rbac-ai-viewer", description="Provision the RBAC AI viewer test user")
    parser.add_argument("--email", default="ai-viewer@acme.com")
