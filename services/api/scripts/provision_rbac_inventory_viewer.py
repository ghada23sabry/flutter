"""Dev-only provisioning for the live inventory RBAC verification user.

Creates a system `inv_viewer` role holding ONLY `inventory.view`, an
`inv-viewer@acme.com` user in the `acme` tenant, and a store-scoped role grant
to the `Downtown` store. Used by `test/live_inventory_test.dart` to prove the
inventory.view / inventory.adjust permission boundary. Idempotent.

Usage (cwd = services/api, .env present):
  python -m scripts.provision_rbac_inventory_viewer
  python -m scripts.provision_rbac_inventory_viewer --email inv-viewer@example.com --password secret
...
VIEWER_PERMISSIONS = ["inventory.view"]
...
async def provision(*, email: str, password: str) -> int:
    async with SessionLocal() as db:
        ...
        role = (
            await db.execute(select(Role).where(Role.tenant_id.is_(None), Role.name == "inv_viewer", Role.is_system.is_(True)))
        ).scalar_one_or_none()
        if role is None:
            role = Role(name="inv_viewer", description="System viewer role (read-only inventory)", is_system=True)
            db.add(role)
            await db.flush()
    ...
    print(f"role=inv_viewer permissions={VIEWER_PERMISSIONS}")
    print(f"user={email} tenant={tenant.slug} store={store.name} store_id={store.id}")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(prog="provision-rbac-inventory-viewer", description="Provision the RBAC inventory viewer test user")
    parser.add_argument("--email", default="inv-viewer@acme.com")
