"""Open Beauty Facts discovery provider.

Searches cosmetics, personal care, and hygiene products via the Open Beauty
Facts API.  Uses the same API structure as Open Food Facts but queries the
...
_OBF_SEARCH_URL = "https://world.openbeautyfacts.org/cgi/search.pl"
_OBF_PRODUCT_URL = "https://world.openbeautyfacts.org/api/v2/product/{barcode}.json"
_OBF_TIMEOUT = 10.0
_PROVIDER_NAME = "open_beauty_facts"


async def search_open_beauty_facts(query: DiscoveryQuery) -> list[DiscoveryCandidate]:
    """Search Open Beauty Facts using barcode or text search."""
...
async def _lookup_barcode(barcode: str) -> list[DiscoveryCandidate]:
    """Direct barcode lookup on Open Beauty Facts."""
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(_OBF_TIMEOUT)) as client:
            resp = await client.get(
                _OBF_PRODUCT_URL.format(barcode=barcode.strip()),
            ...
            if not product:
                return []
            return [_build_candidate(product, barcode=barcode, confidence=0.9, reason="barcode exact match (beauty)")]
    except (httpx.HTTPError, ValueError, KeyError) as exc:
        logger.debug("OBF barcode lookup failed for %s: %s", barcode, exc)
        return []


async def _search_text(search_text: str) -> list[DiscoveryCandidate]:
    """Text search on Open Beauty Facts."""
    params = {
    ...
    try:
        ...
        logger.debug("OBF text search failed for '%s': %s", search_text, exc)
        return []
